<?php
class ControllerPaymentoPayInvoice extends Controller 
{
	private $error = array(); 
	
	public function index() 
	{
		$this->load->language('payment/opayinvoice');
		...
		$data['breadcrumbs'][] = array(
            		...
            		'href' => $this->url->link('payment/opayinvoice', 'token=' . $this->session->data['token'], 'SSL')
        	); 
		...
		$data['action'] = $this->url->link('payment/opayinvoice', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('payment/opayinvoice.tpl', $data));
	}
	
	private function validate() 
	{
		if (!$this->user->hasPermission('modify', 'payment/opayinvoice')) {
			...
			if($nInvoice_Status == 1)
			{
				...
				if(!$bError)
				{
					...
					try
					{
						...
						$opay_invoice->Send['InvoiceRemark'] 			= 'OC2_OPayInvoice_1.1.0212' ;
